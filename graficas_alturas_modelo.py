...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 14 11:17:45 2018
...
#Script creado par ahacer las gráficas de la comparación entre los valores de temperatura en los diferentes días
@author: edwin
"""
...
with open('/media/edwin/6F71AD994355D30E/Edwin/Maestría Meteorologia/Tesis/correccion_alturas.tex','w') as tf: # Tabla de la corrección de temperatura por altura.
    ...
    'correc_1']].to_latex(index = False, longtable = True)) # Forma para escribir una tabla en latex grandísima
...
resumen_1 = pd.merge(resumen, alturas_3, on=['cod','fecha','dom_1'], how='outer') # unión con las alturas para realizar la corrección

resumen_1['temp'] = (resumen_1.T2 + resumen_1.correc_1) # Esta es la adición de la corrección por altura
...
#resumen_1 = pd.read_csv('/media/edwin/6F71AD994355D30E/Edwin/Maestría Meteorologia/Tesis/Extraccion_dominios/resumen_1_20181022.csv')
...
for j in resumen_back.cod.unique()[1:]:#[21201200.0]:#
    ...
    for i in resumen_back.fecha.unique()[1:]:
        ...
        for kk in ['d01','d02','d03']:
            ...
            correccion_1 = resumen_back[(resumen_back.fecha == i) & (resumen_back.cod == j) & (resumen_back.date_1.str.slice(0,3) == kk)].sort_values('date')[['correc_1']].max()
            
            plt.plot_date(comparacion.date, comparacion.tmp_2m, label='Estación')
            plt.plot_date(comparacion.date, comparacion.T2, label='Modelados')
            plt.plot_date(comparacion.date, (comparacion.T2- correccion_1[0]), label='Modelados Corregidos')
            
            plt.xlabel('Fecha')
            plt.xticks(rotation='vertical')
            plt.ylabel('Temperatura °C')
            plt.legend()
            plt.suptitle('Estación '+str(j)+' '+str(i) +' '+'domínio '+kk)
                                
            plt.savefig('/media/edwin/6F71AD994355D30E/Edwin/Maestría Meteorologia/Tesis/grafica_comparacion_temperaturas/' + str(j)+str(i) +kk+'.png', figsize=(20,10) ,dpi = 199)
            plt.close()
...
#                           'fin':[tmp_real.date.max()]})        
    ...
    para_plt = tmp_real[(tmp_real.date > fecha_inicio) & (tmp_real.date < fecha_final)]   
    ...
    plt.plot_date(para_plt.date, para_plt.tmp_2m, '-', color='k')
