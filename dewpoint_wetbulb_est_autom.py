...
Script creado para generar las variables de punto de rocio y de termómetro húmedo para las estaciones automáticas
@author: edwin
"""
    ...
    #    resumen_back.T2 = resumen_back.temp
        ...
        for j in recoleccion_minim_1.cod_1:#[21206990.0]:#
            ...
            if pp_col not in base_validada.columns:
                continue
            # hacer la comparación de wetbulb y de dewpoint
            if varia_1 == '_hum_2m.csv':
               if 'v_'+str(j)[0:-2]+'_tmp_2m.csv' in valores:
                    base_validada2 = pd.read_csv('v_'+str(j)[0:-2]+'_tmp_2m.csv')
                    base_validada2.date =  pd.to_datetime(base_validada2.date, format ='%Y%m%d %H:%M:%S', errors='coerce')
                    base_validada3 = pd.merge(base_validada, base_validada2, on='date', how='inner' )
                    base_validada4 = pd.merge(base_validada, base_validada2, on='date', how='inner' )
                    base_validada3['Td'] = base_validada3.tmp_2m -((100 - base_validada3.hum_2m)/5)#Dewpoint punto de saturación
                    base_validada3['val_td'] = base_validada3.val_hum + base_validada3.val_tmp
                    base_validada3[['Unnamed: 0_x','cod','date', 'Td', 'val_td']]
                    base_validada3.to_csv( 'v_'+str(j)[0:-2]+'_td.csv')
                    ##########
                    base_validada4['wb'] = wb(base_validada4.tmp_2m, base_validada4.hum_2m)
                    base_validada4['val_wb'] = base_validada4.val_hum + base_validada4.val_tmp
                    base_validada4[['Unnamed: 0_x','cod','date', 'wb', 'val_wb']]
                    base_validada4.to_csv( 'v_'+str(j)[0:-2]+'_wb.csv')
